#!/usr/bin/env python3\n"""Strict fast gate for the pre-existing BC1 baseline artifact."""\nfrom __future__ import annotations\n\nimport json\nimport math\nfrom pathlib import Path\n\nROOT = Path(__file__).resolve().parent\nartifact = ROOT / "research" / "BTCUSDT_1h_hypotheses_v1.json"\ndata = json.loads(artifact.read_text(encoding="utf-8"))\n\ndef validate_baseline(data):
    """Validate BC1 from the artifact itself; callers can unit-test mutations without filesystem coupling."""
    ...
    try:
        trade_count = int(metrics["trade_count"]); win_count = int(metrics["win_count"]); loss_count = int(metrics["loss_count"])
        win_rate = float(metrics["win_rate"]); total_return = float(metrics["total_return"])
        avg_return = float(metrics["avg_return"]); profit_factor = float(metrics["profit_factor"]); max_drawdown = float(metrics["max_drawdown"])
    ...
    for name, value in {"win_rate":win_rate,"total_return":total_return,"avg_return":avg_return,"profit_factor":profit_factor,"max_drawdown":max_drawdown}.items():
        if not math.isfinite(value): raise ValueError(f"BC1 validation metric {name} is not finite")
    if trade_count == 0:
        if win_rate != 0.0: raise ValueError("BC1 zero-trade validation must have win_rate=0")
    elif not math.isclose(win_rate, win_count / trade_count, rel_tol=1e-12, abs_tol=1e-12):
