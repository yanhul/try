...
def extract_specific_features(bars: list[MarketBar], window: int = 20) -> list[dict]:
    if window <= 0:
    ...
    for i, b in enumerate(bars):
        ...
        # A zero-range bar against a zero-range history has a neutral spread ratio.
        wide_spread = 0.0 if spread == 0 and range_sma == 0 else ((spread / range_sma) if range_sma else None)
        ...
        absorption_proxy = bool(
            volume_expansion is not None and volume_expansion >= 1.5
            and wide_spread is not None and wide_spread <= 0.8
        )

        price_change = b.close - prev.close
        price_change_pct = (price_change / prev.close) if prev.close else None
        volume_change_pct = ((b.volume - prev_volume) / prev_volume) if prev_volume else None
        price_volume_ratio = (
            abs(price_change_pct) / volume_expansion
            if price_change_pct is not None and volume_expansion else None
        )
        volume_price_expansion = bool(
            volume_expansion is not None and volume_expansion >= 1.5
            and wide_spread is not None and wide_spread >= 1.2
        )
        ...
        sos_proxy = bool(
            volume_expansion is not None and volume_expansion >= 1.5
            and wide_spread is not None and wide_spread >= 1.2 and close_location >= 0.7
        )
        sow_proxy = bool(
            volume_expansion is not None and volume_expansion >= 1.5
            and wide_spread is not None and wide_spread >= 1.2 and close_location <= 0.3
        )

        out.append({
            "timestamp": b.timestamp.isoformat(), "bar_index": i,
            "vsa_volume_expansion": volume_expansion, "vsa_wide_spread": wide_spread,
            "vsa_effort_result": effort_result, "vsa_close_strength": close_location,
            "vsa_up_bar": up_bar, "vsa_down_bar": down_bar, "vsa_volume_up": volume_up,
            "vsa_absorption_proxy": absorption_proxy, "vpa_price_change": price_change,
            "vpa_price_change_pct": price_change_pct, "vpa_volume_change_pct": volume_change_pct,
            "vpa_price_volume_ratio": price_volume_ratio,
            "vpa_volume_price_expansion": volume_price_expansion,
            "wyckoff_spring_proxy": spring_proxy, "wyckoff_upthrust_proxy": upthrust_proxy,
            "wyckoff_sos_proxy": sos_proxy, "wyckoff_sow_proxy": sow_proxy,
