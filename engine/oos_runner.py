...
def main():
    ...
    payload={'schema_version':2,'bc':candidate['bc'],'parent_bc':candidate['parent_bc'],'hypothesis_id':hid,'candidate_hash':candidate['candidate_hash'],'discovery_spec':candidate.get('discovery_spec'),'candidate_universe':candidate_universe,'candidate_family':candidate_family,'dataset':{'path':str(data),'sha256':sha256(data),'bars':len(bars)},'split':{'name':'OOS','start':splits[2].start,'end':splits[2].end},'protocol_sha256':sha256(root/a.protocol),'evaluation_spec':dict(EVALUATION_SPEC),'cost_model':cost_model.metadata(),'execution':{'stop_fraction':EVALUATION_SPEC['stop_fraction'],'reward_multiple':EVALUATION_SPEC['reward_multiple']},'oos_selection_used':False,'oos_executed':True,'predicate':predicate_name,'metrics':m,'trade_count':m.get('trade_count'),'oos_passed':passed}
    out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(payload,indent=2)+'\n'); print(json.dumps({'bc':candidate['bc'],'hypothesis_id':hid,'candidate_universe':candidate_universe,'oos_passed':passed,'metrics':m},indent=2)); return 0
