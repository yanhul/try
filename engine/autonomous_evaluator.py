...
def discovered_predicate(spec):
 ...
 if not math.isfinite(threshold) or spec.get('direction') not in {'above','below'}: raise ValueError('invalid_discovery_threshold')
 def pred(ctx,direction):
  a=series_value(ctx,left); b=series_value(ctx,right) if right else None
  if a is None or (right and b is None):return False
  ...
  return value>threshold if spec['direction']=='above' else value<threshold
 return pred

def mechanism_predicate(spec):
 family=spec['mechanism_family']; threshold=float(spec.get('threshold',0)); direction=spec.get('direction','above')
 if family not in MECHANISMS:raise ValueError('invalid_mechanism_family')
 def pred(ctx,_direction):
  value=series_value(ctx, family)
...
def main()->int:
 ...
 if hid=='discovered_primitive':
  ...
  if not isinstance(candidate.get('discovery_spec'),dict):raise SystemExit('UNEXECUTABLE_MECHANISM_SPEC')
  predicate=mechanism_predicate(candidate['discovery_spec'])
