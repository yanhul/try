...
def _trade_metrics(bars, ledger, stop, rr, *, cost_model: CostModel):
    ...
    trades = [
        Trade(t.ledger_trade.entry_price, t.exit.price, t.ledger_trade.direction.value,
...
def _feature_rows(history, pnf_box_fraction):
    features = extract_features(history); specific = extract_specific_features(history)
    vol = rolling_volatility(history); vw = vwap(history); gann = gann_reference(history)
    mtf = multi_timeframe_context(history); vp_poc = rolling_volume_profile_poc(history)
    ...
    def row(index):
        base = dict(features[index]); base.update(specific[index])
        base.update({"close": history[index].close, "open": history[index].open,
                     "high": history[index].high, "low": history[index].low,
                     "volume": history[index].volume, "volatility": vol[index], "vwap": vw[index]})
        ...
        base["volume_profile_poc"] = vp_poc[index]; base.update(mtf[index])
        base["gann_slope"] = gann[index]["slope"]; base["pnf_direction"] = pnf_direction[index]
        base["momentum_trend"] = mom[index]; base["mean_reversion"] = mr[index]
        base["wyckoff_vsa_vpa"] = vsa[index]["close_location"]; base["regime"] = mtf[index].get("regime", 0)
        base["seasonality"] = history[index].timestamp.weekday(); base["point_figure"] = pnf_direction[index]
...
def _event_context(rows, trade):
    sweep, mss, fvg = rows[trade.sweep_bar], rows[trade.mss_bar], rows[trade.fvg_bar]
    entry = rows[trade.entry_bar].copy(); fvg_context = dict(fvg)
    if trade.fvg_bar >= 2:
        left = rows[trade.fvg_bar - 2]
        if trade.direction == Direction.BULLISH:
            fvg_context["lower"], fvg_context["upper"] = left["high"], fvg["low"]
        else:
            fvg_context["lower"], fvg_context["upper"] = fvg["high"], left["low"]
    return {"sweep": sweep, "mss": mss, "fvg": fvg_context, "entry": entry, "history": rows[: trade.entry_bar + 1]}
...
def prepare_split(bars, start, end, *, pnf_box_fraction=0.01, candidate_universe="reference_event_ledger", candidate_family=None, candidate_spec=None):
    ...
    if candidate_universe == "all_bars":
        ...
        for i in range(max(1, start), end):
            row = rows[i]
            direction = (Direction.BULLISH if family == "discovered_primitive" and (candidate_spec or {}).get("direction") == "above" else Direction.BEARISH if family == "discovered_primitive" and (candidate_spec or {}).get("direction") == "below" else _direction_for_row(row, family))
            if direction is None: continue
            ledger.append(LedgerTrade(direction, i, i, i, i, row["close"])); contexts.append({"entry": row, "history": rows[: i + 1]})
        return PreparedSplit(history=history, ledger=ledger, contexts=contexts, candidate_universe="all_bars")
    events = ReferenceStrategy().process(history); ledger = [t for t in build_ledger(events) if start <= t.entry_bar < end]; contexts = [_event_context(rows, t) for t in ledger]
    return PreparedSplit(history=history, ledger=ledger, contexts=contexts, candidate_universe="reference_event_ledger")


def evaluate_prepared_split(prepared, predicate, stop=0.01, rr=2.0, *, cost_model: CostModel):
    if cost_model is None: raise ValueError("cost_model is required")
    filtered = [t for t, ctx in zip(prepared.ledger, prepared.contexts) if predicate(ctx, t.direction.value)]
    metrics, skipped = _trade_metrics(prepared.history, filtered, stop, rr, cost_model=cost_model)
...
def evaluate_split(bars, start, end, predicate, stop=0.01, rr=2.0, *, prepared=None, pnf_box_fraction=0.01, cost_model: CostModel = DEFAULT_COST_MODEL, candidate_universe="reference_event_ledger", candidate_family=None, candidate_spec=None):
    if cost_model is None: raise ValueError("cost_model is required")
    ...
    return evaluate_prepared_split(prepared, predicate, stop, rr, cost_model=cost_model)


def run_hypothesis_research(csv_path, output_path, *, stop=0.01, rr=2.0, max_components=3, pnf_box_fraction=0.01, cost_model: CostModel = DEFAULT_COST_MODEL):
    if cost_model is None: raise ValueError("cost_model is required")
    bars = load_bars(csv_path); splits = chronological_split(len(bars)); validate_splits(splits, len(bars))
    composites = generate_composites(max_components=max_components); candidates = list(HYPOTHESES.items()) + [(c.name, c.predicate) for c in composites]
    prepared = [prepare_split(bars, split.start, split.end, pnf_box_fraction=pnf_box_fraction) for split in splits]
    result = {"schema_version": 5, "protocol": {"hypothesis_selection": "pre_registered_fixed_rules_and_bounded_composition", "parameter_selection": "none", "execution": "shared_reference_execution", "oos": "descriptive_only_after_is_validation_gate", "max_components": max_components, "max_composites": 120, "feature_evaluation": "causal_split_cache_once_per_split", "pnf_box_fraction": pnf_box_fraction, "cost_model": cost_model.metadata()}, "dataset": {"bars": len(bars), "sha256": _sha256(csv_path)}, "execution": {"stop_fraction": stop, "reward_multiple": rr, "cost_model": cost_model.metadata()}, "hypotheses": {}}
    for name, predicate in candidates:
        ir = evaluate_prepared_split(prepared[0], predicate, stop, rr, cost_model=cost_model); vr = evaluate_prepared_split(prepared[1], predicate, stop, rr, cost_model=cost_model)
        vm = vr["metrics"]; passed, reasons = validation_gate(vm)
        oos = evaluate_prepared_split(prepared[2], predicate, stop, rr, cost_model=cost_model) if passed else None
        result["hypotheses"][name] = {"IS": ir, "VALIDATION": vr, "validation_passed": passed, "validation_gate_reasons": reasons, "OOS": oos}
