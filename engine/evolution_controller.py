...
class EvolutionController:
    ...
    def evaluate(self,candidate:Candidate,hypothesis_id:str="astra")->Evaluation:
        ...
        result=dict(evaluation.result or {});result["candidate"]=canonical_candidate(candidate.config)
        self.ledger.append(ExperimentRecord(candidate.id,hypothesis_id,evaluation.status,candidate.parent_id,configuration_identity=candidate.id,result=result,failure_class=evaluation.failure_class))
        return Evaluation(evaluation.status,evaluation.score,result,evaluation.failure_class)
