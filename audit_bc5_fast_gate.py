...
def main():
    ...
    print("BC5_HYPOTHESIS", {
        "single_change": "liquidity sweep candle body must align with sweep direction",
        "require_sweep_body_alignment": True,
        "rule": "bullish sweep close >= open; bearish sweep close <= open",
        "stop": STOP,
        "target_r": TARGET_R,
        "cost": COST,
    ...
    for split in (is_split, val_split):
        baseline = run_strategy(ReferenceStrategy(False, False), bars, split.start, split.end)
        candidate = run_strategy(ReferenceStrategy(False, True), bars, split.start, split.end)
    ...
    decision = "PROMOTE_TO_FUTURE_OOS_TEST" if all(gates) else "REJECT_BC5"
