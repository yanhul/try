...
def get(url, *, github=False):
    headers={'User-Agent':'try-research-scout/4.0','Accept':'application/json'}
...
def arxiv(q):
    try:
        ...
        req=urllib.request.Request(u,headers={'User-Agent':'try-research-scout/4.0'})
...
def main():
    # The durable archive is authoritative. latest.json is only the latest-run view.
    ...
    payload={'schema_version':5,'fanout':'github+arxiv_parallel_bounded','query_families':len(QUERIES),'queries':QUERIES,'fresh_results':results,'results':merged}
    OUT.parent.mkdir(parents=True,exist_ok=True)
    OUT.write_text(json.dumps(payload,indent=2),encoding='utf-8')
    ARCHIVE.write_text(json.dumps({'schema_version':2,'sources':merged},indent=2),encoding='utf-8')
