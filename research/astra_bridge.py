...
class AstraProposal:
    """A bounded ASTRA mutation before Research evaluation."""
...
def _discovery_mutations(spec: Mapping[str, Any]) -> list[tuple[str, Any]]:
    """Return a deterministic, small neighborhood around executable fields."""
...
def propose_from_research_candidate(parent: Mapping[str, Any], failure: Mapping[str, Any], *, used_ids: set[str] | None = None) -> AstraProposal:
    """Create exactly one deterministic ASTRA child from a failed Research candidate.

    The returned config is an ASTRA search object. It is not a Research verdict.
    No evaluator, OOS result, authority artifact, or policy field is mutable here.
    """
    used = set(used_ids or set())
    spec = parent.get("discovery_spec")
    if not isinstance(spec, Mapping):
        raise ValueError("astra_bridge_requires_discovery_spec")
    if parent.get("candidate_hash") != canonical_hash(dict(parent)):
        raise ValueError("astra_bridge_parent_hash_mismatch")
    base = {"candidate_spec": copy.deepcopy(dict(spec))}
    astra_parent = Candidate(base)
    mutations = [("candidate_spec", {**dict(spec), field: value}) for field, value in _discovery_mutations(spec)]
    if not mutations:
        raise ValueError("astra_bridge_no_executable_mutations")
    for field, value in mutations:
        child = Candidate(mutate(astra_parent.config, field, value), astra_parent.id)
        if child.id in used:
            continue
...
def materialize_research_candidate(parent: Mapping[str, Any], proposal: AstraProposal, *, bc: int) -> dict[str, Any]:
    """Translate an ASTRA proposal back into the strict Research candidate schema."""
    spec = proposal.config.get("candidate_spec")
    if not isinstance(spec, Mapping):
        raise ValueError("astra_bridge_invalid_candidate_spec")
    child = copy.deepcopy(dict(parent))
    child["bc"] = int(bc)
    child["parent_bc"] = int(bc) - 1
    ...
    child["evidence_sources"] = list(parent.get("evidence_sources") or [])
    child["evidence_sources"].append(f"astra:{proposal.astra_parent_id}->{proposal.astra_candidate_id}")
    child["candidate_hash"] = canonical_hash(child)
    return child


def proposal_record(proposal: AstraProposal, research_candidate: Mapping[str, Any]) -> dict[str, Any]:
    """Return immutable lineage metadata for durable recording."""
    return {
        "astra_parent_id": proposal.astra_parent_id,
        "astra_candidate_id": proposal.astra_candidate_id,
        "parent_research_candidate_hash": proposal.parent_research_hash,
        "research_candidate_hash": research_candidate["candidate_hash"],
