...
def classify(item):
 text=_text(item);china=sum(text.count(t) for t in FAMILY_TERMS['china_a_share'])
 if china:return 'china_a_share'
 scores={f:sum(text.count(t) for t in terms) for f,terms in FAMILY_TERMS.items() if f!='china_a_share'};best,score=max(scores.items(),key=lambda x:x[1]);return best if score else 'unknown'
def source_id(item):
...
def normalize(raw):
 family=classify(raw);market='CN_A_SHARE' if family=='china_a_share' else raw.get('market');return {'candidate_id':source_id(raw),'source':raw.get('source','unknown'),'source_url':raw.get('url'),'title':raw.get('title') or raw.get('query'),'description':raw.get('description'),'family':family,'market':market,'query':raw.get('query'),'source_timestamp':raw.get('updated_at'),'lineage':{'source':source_id(raw),'family':family,'rounds':[]}}
def _ensure_normalized(item):
 if 'family' in item and isinstance(item.get('lineage'),dict):return item
 return normalize(item)
def round1_source_quality(items):
 out=[];seen=set()
 for raw in items:
  item=_ensure_normalized(raw);key=item.get('source_url') or re.sub(r'\s+',' ',str(item.get('title','')).lower()).strip()
...
def round2_feasibility(items):
 executable=[];deferred=[]
 for raw in items:
  item=_ensure_normalized(raw);family=item.get('family','unknown');lineage=item.setdefault('lineage',{'rounds':[]});lineage.setdefault('rounds',[])
  if family in EXECUTABLE_FAMILIES:lineage['rounds'].append({'round':2,'decision':'PASS_EXECUTABLE_DATA_LANE'});executable.append(item)
  else:lineage['rounds'].append({'round':2,'decision':'DEFER_DATA_LANE','family':family});deferred.append(item)
...
def _load_discovery_sources():
 sources=[];store='empty'
 if ARCHIVE.exists():
  try:p=json.loads(ARCHIVE.read_text());sources=[x for x in p.get('sources',[]) if isinstance(x,dict) and not x.get('error')];store='archive'
  except (OSError,json.JSONDecodeError,TypeError):pass
 if not sources and LATEST.exists():
  try:p=json.loads(LATEST.read_text());sources=[x for x in p.get('results',[]) if isinstance(x,dict) and not x.get('error')];store='latest_fallback'
  except (OSError,json.JSONDecodeError,TypeError):pass
 if SEEDS.exists():
  try:seed=json.loads(SEEDS.read_text());seed_items=seed.get('sources',[]) if isinstance(seed,dict) else [];sources.extend(x for x in seed_items if isinstance(x,dict));store+='+china_seed'
  except (OSError,json.JSONDecodeError,TypeError):pass
 return sources,store
def build():
 raw,store=_load_discovery_sources();expanded=[]
 for x in raw:expanded.extend(expand_source(x))
 normalized=[normalize(x) for x in expanded];r1=round1_source_quality(normalized);executable,deferred=round2_feasibility(r1);survivors=round3_diversity(executable);payload={'schema_version':4,'architecture':'durable_source_archive + market_seed -> expand -> normalize_dedup -> feasibility -> diversity -> survivor_registry -> executable_translation','source_store':store,'policy_authority':'research/campaign_policy.json','counts':{'raw':len(raw),'expanded':len(expanded),'normalized':len(normalized),'round1':len(r1),'deferred':len(deferred),'survivors':len(survivors),'china_a_share_survivors':sum(x['family']=='china_a_share' for x in survivors)},'population':normalized,'deferred':deferred};POPULATION.write_text(json.dumps(payload,indent=2),encoding='utf-8');SURVIVORS.write_text(json.dumps({'schema_version':4,'status':'SCREEN_SURVIVORS_NOT_PERFORMANCE_PROMOTION','survivors':survivors},indent=2),encoding='utf-8');QUEUE.write_text(json.dumps({'schema_version':4,'status':'READY_FOR_EXECUTABLE_TRANSLATION','candidates':survivors},indent=2),encoding='utf-8');return payload
