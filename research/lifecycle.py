...
STATES=("DISCOVERED","CLAIMED","TRANSLATING","EVALUATING","VALIDATING","OOS_ELIGIBLE","OOS_EXECUTING","EVIDENCE_COMPLETE","DECISION_PENDING","PROMOTED","REJECTED","EXHAUSTED","WAITING","UNKNOWN")
...
TRANSITIONS={
 ...
 "CLAIMED":{"TRANSLATING","EVALUATING","WAITING","UNKNOWN","REJECTED"},
...
class LifecycleEvent:
 campaign_id:str; candidate_id:str; research_work_id:str; attempt_id:str|None; from_state:str|None; to_state:str; reason_code:str; evidence_refs:tuple[str,...]=(); parent_id:str|None=None; decision_id:str|None=None; timestamp:str=""
 def __post_init__(self)->None:
  if self.to_state not in STATES: raise ValueError(f"invalid_lifecycle_state:{self.to_state}")
...
class LifecycleStore:
 ...
 def current(self,candidate_id:str)->str|None:
  ...
  for event in self.read():
   if event.get("candidate_id")==candidate_id:state=event.get("to_state")
  return state
 def transition(self,event:LifecycleEvent)->str:
  current=self.current(event.candidate_id)
  if current!=event.from_state:raise ValueError(f"lifecycle_state_conflict:{current!r}->{event.to_state!r}")
  if event.from_state is not None and event.to_state not in TRANSITIONS.get(event.from_state,set()):raise ValueError(f"invalid_lifecycle_transition:{event.from_state}->{event.to_state}")
  payload=asdict(event);payload["evidence_refs"]=list(event.evidence_refs);payload["event_id"]=event.event_id
  with self.path.open("a",encoding="utf-8") as handle:handle.write(json.dumps(payload,sort_keys=True,ensure_ascii=False)+"\n");handle.flush()
  return event.event_id
 def ensure_discovered(self,campaign_id:str,candidate_id:str,research_work_id:str,*,parent_id:str|None=None)->str:
  if self.current(candidate_id) is None:return self.transition(LifecycleEvent(campaign_id,candidate_id,research_work_id,None,None,"DISCOVERED","CANDIDATE_DISCOVERED",parent_id=parent_id))
  return self.current(candidate_id) or "DISCOVERED"
def new_attempt_id(candidate_id:str,attempt_no:int)->str:return hashlib.sha256(f"{candidate_id}:{attempt_no}".encode()).hexdigest()
