...
def btc_translation_status(candidate: dict) -> tuple[bool,str]:
    ...
    if market and market not in {"CRYPTO","CRYPTOCURRENCY","DIGITAL_ASSET","BTC","BTCUSDT"}:
        if any(x in text for x in ("cross-sectional","cross sectional","portfolio","factor","alpha factors")):return False,f"single_asset_incompatible:{market}"
