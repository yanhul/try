...
The lifecycle decides *what* transition is legal; this module decides whether an
execution effect has enough predecessor/evidence/authority/lineage to be
permitted.  Executors must require the resulting permit.
"""
...
def authorize(
    *,
    ...
    """Return ALLOW only when every execution precondition is valid."""
    checks = (
        ("BLOCKED", not blocked),
        ...
        ("MISSING_IDEMPOTENCY_KEY", bool(idempotency_key)),
    )
...
def require_permit(permit: Permit) -> None:
    """Hard execution boundary: a denied permit cannot cross into execution."""
