...
def config():
 base="https://generativelanguage.googleapis.com/v1beta/openai/".rstrip("/");return base,os.getenv("GEMINI_MODEL","gemini-3.1-flash-lite"),os.getenv("GEMINI_API_KEY","")
def interval():
...
def call(prompt):
 ...
 for attempt in range(retries+1):
  try:
   ...
   delay=min(float(os.getenv("RESEARCH_PROVIDER_BACKOFF_CAP_SECONDS","120")),2.0**attempt+random.uniform(0,1));time.sleep(delay);_PROVIDER_LAST_CALL=time.monotonic()
...
def main():
 ...
 try:
  q=json.loads(queue.read_text(encoding="utf-8"));raw=q.get("candidates",[]) if isinstance(q,dict) else q;survivors=[s for s in raw if isinstance(s,dict) and str(s.get("source_url") or "").strip()];eligible,rejected=eligible_survivors(survivors);eligible=[s for s in eligible if str(s.get("family") or "").strip() in EXECUTABLE_MECHANISM_FAMILIES]
  ...
  ranked=rank_families(families,seed=bc);family=ranked[0];selected=next(s for s in eligible if str(s.get("family") or "").strip()==family);selected_family=family
 except Exception as e:print(f"PROVIDER_ROUTER_HOLD malformed_screen_queue:{e}");return 0
 forbidden=prior_fingerprints();failure_text=compact(failure.read_text(encoding="utf-8"));evidence=survivor_evidence(selected);forbidden_payload=json.dumps([list(x) for x in sorted(forbidden,key=str)],separators=(',',':'));prompt=(f"Parent BC: {parent}\nNext BC: {bc}\nTARGET_MARKET: BTCUSDT\nTARGET_TIMEFRAME: 1H\nSELECTED_SURVIVOR_FAMILY: {json.dumps(selected_family)}\nALLOWED_MECHANISM_FAMILY_FOR_THIS_REQUEST: {json.dumps(selected_family)}\nFORBIDDEN_STRUCTURAL_MECHANISMS_COMPLETE: {forbidden_payload}\nFAILURE ANALYSIS (repair context only, never evidence):\n{failure_text}\nSELECTED SCREEN SURVIVOR (authoritative; translate this one only):\n{json.dumps(selected,sort_keys=True,separators=(',',':'))}\nTranslate the mechanism faithfully to BTCUSDT 1H without changing asset, data lane, or selected family. If no genuinely different OHLCV expression is supported, return HOLD.")
 try:
