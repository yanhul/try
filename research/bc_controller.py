...
def main():
 ...
 if PROMOTE in out:
  ...
  if receipt_events[0].get('receipt_type')!=receipt_binding['receipt_type'] or receipt_events[0].get('receipt_schema_version')!=receipt_binding['receipt_schema_version'] or receipt_events[0].get('receipt_id')!=receipt_binding['receipt_id']:
   return hold(s,'HOLD_OOS_RECEIPT_EVENT_BINDING_MISMATCH',bc,retryable=False)
  assert_history_entry_legal(receipt_events[0])
