...
def save(s):
 s['updated_at']=datetime.now(timezone.utc).isoformat(); STATE.write_text(json.dumps(s,indent=2,sort_keys=True)+'\n',encoding='utf-8')
...
def main():
 ...
 for _ in range(MAX):
  ...
  s['last_bc']=bc; s['iterations']=int(s.get('iterations',0))+1; checkpoint(s,'ACT',bc)
  print(f'CONTROLLER_CANDIDATE BC{bc} hypothesis_id={c["hypothesis_id"]} GATE {g.name}')
  ...
  if PROMOTE in out:
   ...
   s['terminal']=True; s['terminal_reason']=decision; s['next_bc']=bc+1; write_queue([]); checkpoint(s,'TERMINAL',bc)
  ...
  write_queue([]); s['history'].append({'bc':bc,'decision':'REJECT','next':'AGENT_HYPOTHESIS','hypothesis_id':c['hypothesis_id']}); s['next_bc']=bc+1; checkpoint(s,'PERSISTED',bc)
