...
def regenerate(bc,parent,failure):
 ...
 # Provider failure/hold is a hard stop. An old artifact must never count as success.
...
def main():
 ...
 for _ in range(MAX):
  ...
  if not valid:
   ...
   load_candidate(candidate,bc,parent); c=json.loads(candidate.read_text(encoding='utf-8')); q[0]=c; QUEUE.write_text(json.dumps(q,indent=2,sort_keys=True)+'\n',encoding='utf-8')
  ...
  q=load(QUEUE,[])[1:]; QUEUE.write_text(json.dumps(q,indent=2,sort_keys=True)+'\n',encoding='utf-8'); s['history'].append({'bc':bc,'decision':'REJECT','next':'AGENT_HYPOTHESIS'}); save(s)
