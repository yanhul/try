...
def test_campaign_persists_and_resumes(tmp_path, monkeypatch):
    ...
    first = run_campaign("data/BTCUSDT_1h.csv", ledger, state, max_generations=1, generation_limit=6)
    assert first.terminal is True
    assert first.generation == 1
    assert load_state(state).parent["reward_multiple"] == 2.5
    calls_after_first = len(calls)

    resumed = run_campaign("data/BTCUSDT_1h.csv", ledger, state, max_generations=3, generation_limit=6)
