...
def _result(passed):
    return {
        "metrics": {
            "profit_factor": 1.2 if passed else 0.4,
            "total_return": 0.1 if passed else -0.2,
            "max_drawdown": 0.2 if passed else 0.8,
            "trades": 20,
        }
    }
...
def test_ranking_is_deterministic_for_same_memory_and_seed():
    assert rank_families(["momentum_trend", "mean_reversion"], seed=7) == rank_families(
        ["momentum_trend", "mean_reversion"], seed=7
    )


def test_record_is_idempotent_and_excludes_harness_failures(monkeypatch, tmp_path):
    memory = tmp_path / "search_memory.json"
    monkeypatch.setattr(search_memory, "MEMORY_PATH", memory)
...
def test_stagnation_changes_search_regime_and_prioritizes_unseen_family(monkeypatch, tmp_path):
    ...
    ranked = search_memory.rank_families(["family_a", "family_b"], seed=9)
    assert ranked[0] == "family_b"
...
def test_rebuild_from_durable_oos_artifacts_records_learning(monkeypatch, tmp_path):
    ...
    (oos_dir / "BC3_oos_result.json").write_text(
        json.dumps({"oos_executed": True, "oos_passed": False, "metrics": {"profit_factor": 0.5}}),
        encoding="utf-8",
    )
